...
var generatelist = function(){//生成播放列表+播放
	var list = [
		{
		title:"II snow is falling",
		mp3:"http://storage.live.com/items/48BC8CF9CA8EF426!1111",
		ogg:"http://storage.live.com/items/48BC8CF9CA8EF426!1112"
		},
		{
		title:"cry for me,cry for you",
		mp3:"http://storage.live.com/items/48BC8CF9CA8EF426!1110",
		ogg:"http://storage.live.com/items/48BC8CF9CA8EF426!1109"
		},
		{
		title:"pocket",
		mp3:"http://storage.live.com/items/48BC8CF9CA8EF426!1108",
		ogg:"http://storage.live.com/items/48BC8CF9CA8EF426!1113"
		}
	]
	...
	var durationchange = function(){
		setInterval("var jsprogress2 = document.getElementById('jsprogress2');\
			...
			jsprogress2.style.width = (width * 100) + '%';",100)
	}
...
$gavatar_url = "http://0.gravatar.com/avatar/"+$gavatar;
$("#ajaxgravatar").html('<img alt="Gravatar" src="'+ $gavatar_url +'?s=45"/>')
	...
	if($("#comment").length){
		...
		if($("#email").length){
			var loadavatar = function(){
				$("#ajaxgravatar").html('<img alt="Gravatar" src="'+ "http://0.gravatar.com/avatar/" + hex_md5($.trim($("#email").val()).toLowerCase()) +'?s=40"/>')
			}
			loadavatar();
...
(function($) {
	...
	var pjax = function(options) {
		...
		$('body').delegate(options.selector, 'click', function(event) {
			...
			if(href.indexOf('#') > 0 && href.indexOf(window.location.href.slice(0,-window.location.hash.length)) == 0){
				return true;//这是我自己写的“只是hash不同”，welefeng老师原版的看不懂……（by ttfioc）
			}
...
//  			}
	...
	pjax.defaultOptions = {
		...
		beforeSend : function(xhr) {
			...
			ajaxloading.innerHTML = "<a href=\"#\" title=\"click to open it without ajax loading\"><div></div></a>";
			document.body.appendChild(ajaxloading);
			$("#ajaxloading").animate({opacity:"0.6"});
			$("#ajaxloading div")[0].style.backgroundPosition = "3000px center";
		...
		complete : function(xhr) {
			...
			setTimeout('$("#loadbar2").animate({opacity:"0"});',500);
			setTimeout('$("#loadbar2").css("width","0");',1200);
		}
	};
	// 展现动画
	pjax.showFx = {
